...
//    d_a.copyFrom(a.ptr());
    ...
    occa::memory d_a = platform.malloc<Real>(a); // allocate device memory; copy host memory
    occa::memory d_b = platform.malloc<Real>(b); // allocate device memory/initialize value
    occa::memory d_ab = platform.malloc<Real>(nentries);    // allocate device memory

    // device function execution
    solver->addVectors(nentries,d_a,d_b,d_ab);

    // copy device data to host
    d_ab.copyTo(ab.ptr(),ab.size());
