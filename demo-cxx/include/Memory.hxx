...
class memory {
  ...
  public:
   ...
   ~memory()=default;
    ...
    void dump(const int nlines = -1) {
        int cnt = (nlines == -1) ? (int) lngth : nlines;
        cnt = std::min(cnt,(int)lngth);
    ...
    void L2(const int nlines = -1) {
        int cnt = (nlines == -1) ? (int) lngth : nlines;
        cnt = std::min(cnt,(int)lngth);

        double sum = 0.0;
        for (int i=0;i<cnt;++i) sum += shrdPtr[i];
        sum /= (double) cnt;
        printf("L2: %.15e\n",sum);
    }

    void minmax(const int nlines = -1) {
        int cnt = (nlines == -1) ? (int) lngth : nlines;
        cnt = std::min(cnt,(int) lngth);
...
/* amc::deviceMemory is a wrapper around occa::memory */
template<typename T>
class deviceMemory: public occa::memory {
  public:
   ...
   ~deviceMemory()=default;
    ...
    /* copy from amc::memory */
    void copyFrom(const amc::memory<T> src,
                  const ptrdiff_t count = -1,
    ...
    void zero(){
        amc::memory<T> dest(length(),0);
        copyFrom(dest); // copy Host2Device
        dest.free();    // delete host memory
    }

    void dump(int nlines=-1){
        const int cnt = (nlines == -1) ? (int) length():nlines;
        amc::memory<T> dest(length());
